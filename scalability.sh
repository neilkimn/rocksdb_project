...
SEED=1234
ROOT_PATH=scalability

for TOTAL_KEYS in 1000000 2000000 5000000
do
    BASE_PATH=$ROOT_PATH/$TOTAL_KEYS"_keys"
    mkdir -p $BASE_PATH
    ...
    do
        ...
        if [[ $TOTAL_KEYS -eq 1000000 ]] && [[ $NUM_THREADS -eq 1 ]]; then
            ...
            do
                ...
                time ./rocksdb/db_bench -benchmarks=fillrandom -use_existing_db=0 -db=DBs -wal_dir=DBs -key_size=16 -value_size=100 -num=$TOTAL_KEYS -compression_type=none -threads=$NUM_THREADS -seed=$SEED \
                        >$VAR_PATH"/write_random_run_"$i".log" 2>&1;
            done
        fi
